...
def trim(in_file, out_file, start, end):
    ...
    audio_stream = next(
        (s for s in in_file_probe_result["streams"] if s["codec_type"] == "audio"), None
    )
    if audio_stream:
...
def main(
    video_repos: str = "/raid/datasets/msr-vtt/TrainValVideo",
    p_save: str = "/raid/datasets/msr-vtt/Train_scene_mp4/",
    error_path: str = "/srv/home/ahmadi/gitrepos/video-ir/error_paths.txt",
):
    path_array = []
    ...
    for file in tqdm(path_array, desc="Iterate over each video file..."):
        flag = False
        if file in f:
            flag = True
